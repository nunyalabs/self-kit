...
let isPaused = false;
export const lastBlobByMode = { idi: null, questionnaire: null };
export const lastFilenameByMode = { idi: null, questionnaire: null };
let audioCtx;
...
function getSelectors(mode='idi') {
  return {
    recordBtn: mode === 'questionnaire' ? 'singleRecordBtn' : 'idiRecordBtn',
    stopBtn: mode === 'questionnaire' ? 'singleStopBtn' : 'idiRecordBtn', // In questionnaire, stop is separate
  ...
  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(stream => {
      ...
      mediaRecorder.onstop = () => {
        ...
        let participantId = '';
        let studySite = '';
        participantId = document.getElementById(mode === 'questionnaire' ? 'participantId' : sels.participantInput)?.value || '';
        if (mode === 'questionnaire') {
          const siteSelect = document.getElementById('studySite');
          if (siteSelect) {
            studySite = siteSelect.value === 'Other' ? document.getElementById('studySiteOther')?.value : siteSelect.value;
          }
        }
        const interviewType = mode;
        lastBlobByMode[mode] = blob; // Store the blob
        lastFilenameByMode[mode] = buildRecordingFilename(mode, participantId, blob.type, studySite); // Pass study site
  const dlBtn = document.getElementById(mode === 'questionnaire' ? 'singleDownloadBtn' : 'idiDownloadBtn');
...
function buildRecordingFilename(mode, participantIdJoined, mime, site) {
  const date = new Date();
  ...
  // For questionnaire mode, use the passed site. For IDI, look it up.
  const loc = sanitize(site || 'UnknownSite');
